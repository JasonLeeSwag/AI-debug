...
**[前置]** **環境脈絡收集（強制）**：任何測試執行前，必須先執行 `scripts/env_context.py` 收集並記錄當下的 OS、裝置類型、網路類型（WiFi／有線／4G/5G）、VPN 狀態、IP 地理位置。環境脈絡是 Bug report 的必要欄位——同一 Bug 在 VPN 開啟、弱網路、特定 OS 下行為可能完全不同。
...
> 環境脈絡警告：支付回調測試執行前，若偵測到 VPN 啟用，必須提示 QA 確認是否繼續（部分支付閘道有 IP 白名單限制）。
